...
def retrieve_onet_titles():
    ...
    onet_titles = pd.concat(
        (pd.read_csv(f, sep='\t') for f in all_occupation_files),
        ignore_index=True
    )
...
def concept_rank(words, tfidf_vectorizer, tf, concept_row):
    """
...
def normalize_job_title(job_title):
    ...
    onet_titles = retrieve_onet_titles()
    # Transform
    ...
    # for inspection, not required
    # inverse_vocabulary = {v: k for k, v in tfidf_vectorizer.vocabulary_.items()}
    ...
    for title in hypernym_product(job_title):
        for concept in concept_rank(title, tfidf_vectorizer, tf, concept_row):
