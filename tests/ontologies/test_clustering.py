...
class TestClustering(unittest.TestCase):
    def test_basic(self):
        ...
        major_group_37_entities = onet.filter_by(lambda edge: edge.occupation.identifier[:3] == '37-').occupations

        major_group_35_concept= list(onet.filter_by(lambda edge: edge.occupation.identifier == '35').occupations)[0]
        major_group_35_entities = onet.filter_by(lambda edge: edge.occupation.identifier[:3] == '35-').occupations
        ...
        assert occupation_clustering.map_original_key["Building and Grounds Cleaning and Maintenance"] == major_group_37_concept
        assert occupation_clustering["Food Preparation and Serving Related Occupations"] == major_group_35_entities
        assert occupation_clustering.map_original_key["Food Preparation and Serving Related Occupations"] == major_group_35_concept
